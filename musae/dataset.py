...
class MidiDataset():
	...
	def preprocess_dataset3(self, pianorolls_folder, metadata_folder, early_exit):
		...
		with Progress() as progress:
			task = progress.add_task("Preprocessing songs...", total=self.dataset_length)
	...
	def preprocess_dataset_clean(self, data_folder, early_exit=None, n_processes=5):
		...
		# count number of files of dataset (slow but ok)
		# self.dataset_length = sum([len(files) for _, _, files in os.walk(data_folder)])
		...
		with Progress() as progress:
			...
			# 								tmp.transpose(shift)
			# 								tmp.check_validity()
			# 								tmp.save(os.path.join(processed_folder, str(yeah) + ".npz"))
			# 								del tmp
			# 								store_meta = True
			# 								# adding link to corresponding metadata file
			# 								fetch_meta[str(yeah)] = pbc
			...
			#
			# 	progress.console.print("pbc:", pbc)
			# 	progress.console.print("yeah:", yeah)
	...
	def extract_real_song_names(self, pianorolls_folder, metadata_folder, early_exit):
		...
		with Progress() as progress:
			task = progress.add_task("Extracting real song names...", total=self.dataset_length)
